...
// and fill them in with all your shape drawing calls and any extra key / mouse controls.
    ...
    'display'( graphics_state )
      { var model_transform = identity();             // We have to reset model_transform every frame
        ...
        **********************************/  
        ...
        function animation(model_transform) {
  ...
  //  model_transform = mult(model_transform, rotation(60 * Math.sin(graphics_state.animation_time/2000), 1, 0, 0));
  //       this.shapes.sphere.draw(graphics_state, model_transform, this.bluePlastic);
...
var model_transform = identity();
  var x_wing_width= 5,
    ...
    rotation_angle = 90,
		flapping_angle = Math.sin(graphics_state.animation_time/500) * 0.7;

// model_transform = mult(model_transform, translation(4,4, 1));


  model_transform = mult(model_transform, translation(0,-5, 6));
 model_transform = mult(model_transform, rotation(rotation_angle * flapping_angle, 1,0,0));
 model_transform = mult(model_transform, translation(0,0.4, 4));
     model_transform = mult(model_transform, scale(x_wing_width, y_wing_height, z_wing_length));
                this.shapes.box.draw(graphics_state, model_transform, this.bluePlastic);
  ...
  { 'construct'( context )
      { var shapes = { "box" : new Cube(), 
        ...
        this.define_data_members( { yellow_clay: context.shaders_in_use["Phong_Model"].material( Color(  1,  1, .3, 1 ), .2, 1, .7, 40 ),
                                     ...
                                     darkBrown : context.shaders_in_use["Phong_Model" ].material( Color( .4,.4,.5, 9 ), .9, .0, .0, 20),
                                     guess1 : context.shaders_in_use["Phong_Model" ].material( Color( .1,.7,.8, 9 ), .9, .5, .8, 20),
...
'draw_background' (model_transform, graphics_state) {
  model_transform = mult(model_transform, translation(0,-212,0));
      model_transform = mult(model_transform, scale(200, 200, 20)); 
         this.shapes.box.draw(graphics_state, model_transform, this.green);
  
},

'create_head' (model_transform, head_radius, graphics_state) {
          ...
          model_transform = mult(model_transform, scale(head_radius, head_radius, head_radius));
            this.shapes.ball.draw(graphics_state, model_transform, this.dark);
          model_transform = mult(model_transform, scale(1/head_radius, 1/head_radius, 1/head_radius));
    },

/////////////////////////////////////****************Bee body*************///////////////////////

    'create_body' (model_transform, graphics_state, body_length, body_width, body_height, head_radius) {
       model_transform = mult(model_transform, translation( 2.8 * head_radius, 0, 0));
            model_transform = mult(model_transform, scale(body_length, body_width, body_height));
              this.shapes.box.draw(graphics_state, model_transform, this.unknown1);
          model_transform = mult(model_transform, scale(1/body_length, 1/body_width, 1/body_height));
        model_transform = mult(model_transform, translation(- (2.8 * head_radius), 0, 0));
...
//////////////////***************bees Tail***************/////////
    ...
    'create_tail' (model_transform, graphics_state, tail_length, tail_width, tail_height) {
        model_transform = mult(model_transform, translation(5.8, 0, 0));
            model_transform = mult(model_transform, scale(tail_length, tail_width, tail_height));
              this.shapes.ball.draw(graphics_state, model_transform, this.darkWhite);
            model_transform = mult(model_transform, scale(1/tail_length, 1/tail_width, 1/tail_height))
          model_transform = mult(model_transform, translation(-5.8, 0, 0));
    },

 /////////////***************** bee's wings *******************/////////////////////////
            ////////////************************** Left Wing *****************************///////////
  'create_left_wing' (model_transform, graphics_state, x_wing_width, y_wing_height, z_wing_length, rotation_angel, flapping_angle, head_radius) {
    model_transform = mult(model_transform, translation(head_radius * 2 + 0.5, 0.7, 0.7));
    model_transform = mult(model_transform, rotation(rotation_angle * flapping_angle, 1,0,0));
      model_transform = mult(model_transform, translation(0, 0.1, 1.6));
        model_transform = mult(model_transform, scale(x_wing_width, y_wing_height, z_wing_length));
                this.shapes.box.draw(graphics_state, model_transform, this.darkBrown);
              model_transform = mult(model_transform, scale(1/x_wing_width, 1/y_wing_height, 1/z_wing_length));
          model_transform = mult(model_transform, translation(0,-0.1,-1.6));
      model_transform = mult(model_transform, rotation((rotation_angle * flapping_angle), -1,0,0))
model_transform = mult(model_transform, translation(-(head_radius * 2 + 0.5), -0.7, -0.7));
},
...
'create_right_wing' (model_transform, graphics_state, x_wing_width, y_wing_height, z_wing_length, rotation_angel, flapping_angle, head_radius) {
   model_transform = mult(model_transform, translation(head_radius * 2 + 0.5, 0.7, -0.7));
    model_transform = mult(model_transform, rotation(rotation_angle * flapping_angle, 1,0,0))
       model_transform = mult(model_transform, translation(0, 0.1, -1.6));
         model_transform = mult(model_transform, scale(x_wing_width, y_wing_height, z_wing_length));
...
'create_left_legs' (model_transform, graphics_state, legs_length, legs_height, legs_width, rotation_angel, upper_leg_rotation, leg_swing_angle, lower_leg_rotation, lower_leg_swing_angle) {
   
   for (var i = 0; i < 3; i ++) {
 model_transform = mult(model_transform, rotation(rotation_angel, 1, 0, 0,0))
        model_transform = mult(model_transform, translation(0, 0.7 ,0.7 ));
          model_transform = mult(model_transform, rotation(upper_leg_rotation * leg_swing_angle, 1, 0, 0)); //upper leg rotation angel
              model_transform = mult(model_transform, translation(1.05, legs_height ,legs_width));
                  model_transform = mult(model_transform, scale(legs_length, legs_height, legs_width));
                    this.shapes.box.draw(graphics_state, model_transform, this.darkBrown);
                  model_transform = mult(model_transform, scale(1/legs_length, 1/legs_height, 1/legs_width));
         
         model_transform = mult(model_transform, translation(0, 0.6 ,0.2 )); //pivot at the end of the first leg

              //LOWER
      model_transform = mult(model_transform, translation(0, 0.1,-0.4));
         model_transform = mult(model_transform, rotation(lower_leg_rotation * lower_leg_swing_angle, 1, 0, 0)) //lower leg rotation angel
            model_transform = mult(model_transform, translation(0, legs_height,legs_width ));
                model_transform = mult(model_transform, scale(legs_length, legs_height, legs_width));
                    this.shapes.box.draw(graphics_state, model_transform, this.darkBrown);
                model_transform = mult(model_transform, scale(1/legs_length, 1/legs_height, 1/legs_width));
             model_transform = mult(model_transform, translation(0, -legs_height,-legs_width ));
       model_transform = mult(model_transform, rotation(-lower_leg_rotation * lower_leg_swing_angle, 1, 0, 0));
   model_transform = mult(model_transform, translation(0, -0.1, 0.4));
   
   /////getting back to the original co-ordinate started with     
                         model_transform = mult(model_transform, translation(0, -0.6, -0.2));
                    model_transform = mult(model_transform, translation(-1.05, -0.7 , -0.2 )); 
                 model_transform = mult(model_transform, rotation(-upper_leg_rotation * leg_swing_angle, 1, 0, 0)); 
            model_transform = mult(model_transform, translation(0, -0.7 , -0.7 ));
          model_transform = mult(model_transform, translation(1.1, 0, 0)); //1.1 is leg seperation distance
  model_transform = mult(model_transform, rotation(rotation_angel, -1, 0, 0,0));
   }
...
'create_right_legs' (model_transform, graphics_state, legs_length, legs_height, legs_width, rotation_angel, upper_leg_rotation, leg_swing_angle, lower_leg_rotation, lower_leg_swing_angle) {
  ...
  model_transform = mult(model_transform, rotation(-lower_leg_rotation * lower_leg_swing_angle, 1, 0, 0)) //lower leg rotation angel
   ...
   model_transform = mult(model_transform, rotation(lower_leg_rotation * lower_leg_swing_angle, 1, 0, 0));
   /////getting back to the original co-ordinate started with     
                         model_transform = mult(model_transform, translation(0, -0.7, -0.2));
                    model_transform = mult(model_transform, translation(-1.05, -0.7 , 0.2 )); 
                 model_transform = mult(model_transform, rotation(upper_leg_rotation * leg_swing_angle, 1, 0, 0)); 
            model_transform = mult(model_transform, translation(0, -0.6 , 0.7 ));
          model_transform = mult(model_transform, translation(1.1, 0, 0)); //1.1 is leg seperation distance
  model_transform = mult(model_transform, rotation(rotation_angel, -1, 0, 0,0));
} 
},
 ...
 ////////////////******************************** Flower & Stem ******************************///////////////////////////////////
      //////////////---------------STEAM------------ /////////////////////////////////////////////////////
'draw_stem' (model_transform, graphics_state, stem_width, stem_height, stem_length, x_axis, y_axis, rotation_angel, graphic_time) {
  for (var i = 0; i < 8; i++) {

  model_transform = mult(model_transform, translation(0, stem_height, 0));
    model_transform = mult(model_transform, rotation(graphic_time * rotation_angel, 0,0,1));  
        model_transform = mult(model_transform, translation(0, stem_height, 0));
          model_transform = mult(model_transform, scale(stem_length, stem_height, stem_width));
          this.shapes.box.draw(graphics_state, model_transform, this.guess);
      model_transform = mult(model_transform, scale(1/stem_length, 1/stem_height, 1/stem_width));
}
...
/////////////////////*********************/////////////////////////////////       
        ...
        graphics = (graphics_state.animation_time/2000) % 360,
		    body_displace_x = -6 * Math.sin(graphics),
	     	body_displace_z = 6* Math.cos(graphics),
		    body_displace_y = 0,
...
model_transform = mult(model_transform, translation(body_displace_x,body_displace_y,body_displace_z));   
  model_transform = mult(model_transform, translation(0, 2 *Math.sin(graphics*2),0));
    model_transform = mult(model_transform, rotation(rotation_deg, 0, -1,0));

      
      //creating the bee head   
  this.create_head(model_transform, head_radius, graphics_state);
         
  model_transform = mult(model_transform, translation(-6,-2,0)); //model_transform has been changed

/////////////////////////////////////****************Bee body*************///////////////////////
var body_length = 1.5,
    body_width = 0.7,
    body_height = 0.7;   
    this.create_body(model_transform, graphics_state, body_length, body_width, body_height, head_radius);
...
var tail_length = 2.2,
  ...
  this.create_tail (model_transform, graphics_state, tail_length, tail_width, tail_height);
...
var x_wing_width= 1,
  ...
  this.create_left_wing (model_transform, graphics_state, x_wing_width, y_wing_height, z_wing_length, rotation_angel, flapping_angle, head_radius);


// // ////////////////////////********************** Right Wing ******************************** ///////////////////////
	flapping_angle = -Math.sin(graphics_state.animation_time/500) * 0.7;

  this.create_right_wing (model_transform, graphics_state, x_wing_width, y_wing_height, z_wing_length, rotation_angel, flapping_angle, head_radius);
...
///////////////*************************** Legs ******************************///////////////
            ...
            var legs_width = 0.2,
                ...
                legs_length = 0.2,
                rotation_angel = 180,
                leg_swing_angle = -(Math.sin(graphics_state.animation_time/500))* 0.5,
                lower_leg_swing_angle = -(Math.cos(graphics_state.animation_time/500) + 1)* 0.5,
                upper_leg_rotation = 45,
                lower_leg_rotation = 30,

                legs_distance = 0;

                ////////////******************* LEFT-LEGS ******************
this.create_left_legs (model_transform, graphics_state, legs_length, legs_height, legs_width, rotation_angel, upper_leg_rotation, leg_swing_angle, lower_leg_rotation, lower_leg_swing_angle);     

// // // //////////*************RIGHT- LEGS**********************/////////////////

this.create_right_legs (model_transform, graphics_state, legs_length, legs_height, legs_width, rotation_angel, upper_leg_rotation, leg_swing_angle, lower_leg_rotation, lower_leg_swing_angle);   
...
//2'nd  co-ordinate 
    ...
    ////////////////******************************** Flower & Stem ******************************///////////////////////////////////
      ...
      var model_transform = identity(); //making identity matrix
      var stem_width = 0.3,
      stem_height = 0.7,
      stem_length = 0.3,
      ...
      y_axis = -8,
      rotation_angel = 5,
		  graphic_time = Math.sin(graphics_state.animation_time/400) * 0.8;

      model_transform = mult(model_transform, translation(x_axis , y_axis ,0 )); //stem co-ordinate

      this.draw_stem (model_transform, graphics_state, stem_width, stem_height, stem_length, x_axis, y_axis, rotation_angel, graphic_time); 

//////// drawing a flower /////////////////////////////
model_transform = this.draw_stem (model_transform, graphics_state, stem_width, stem_height, stem_length, x_axis, y_axis, rotation_angel, graphic_time); 
