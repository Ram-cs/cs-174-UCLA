...
Declare_Any_Class( "Octagon_cone",    // First, the simplest possible Shape – one triangle.  It has 3 vertices, each having their own 3D position, normal
  { 'populate'()                  // vector, and texture-space coordinate.
     
     { this.positions      = [ vec3(0,0,1), vec3(1,0,0), vec3(0.66,0.8,0), 
                                vec3(0,1,0), vec3(-0.66,0.8,0), vec3(-1,0,0), 
                                vec3(-0.66,-0.8,0), vec3(0,-1,0), vec3(0.66,-0.8,0)];   // Specify the 9 vertices -- the point cloud that our Octagon needs.
        this.normals        = [ vec3(0,0,1), vec3(0,0,1), vec3(0,0,1), 
                                vec3(0,0,1),vec3(0,0,1), vec3(0,0,1), 
                                vec3(0,0,1), vec3(0,0,1),vec3(0,0,1)  ];   // ...
        this.texture_coords = [ vec2(0.5,0.5),   vec2(1,0.5),   vec2(0.8,0.8), 
                                vec2(0.5,1),vec2(0.3,0.8),   vec2(0,0.5),   
                                vec2(0.3,0.3), vec2(0.5,0),vec2(0.8,0.3)];   // ...
        this.indices        = [ 0,1,2,  0,2,3,   0,3,4,  
                                0,4,5,    0,5,6,   0,6,7,  
                                0,7,8,    0,8,1,   1,2,8,     
                                2,7,3,    2,7,8,   3,5,4,  
                                3,6,5,    3,7,6];            
  ...
  Declare_Any_Class( "test_pyramid",    // First, the simplest possible Shape – one triangle.  It has 3 vertices, each having their own 3D position, normal
  { 'populate'()                  // vector, and texture-space coordinate.
     
     { this.positions      = [vec3(0,0,1), 
                              vec3(1,0,0), vec3(0,1,0), vec3(-1,0,0), 
                              vec3(0,-1,0)];   // Specify the 5 vertices -- the point cloud that our Octagon needs.
        this.normals        = [ vec3(0,0,1), vec3(0,0,1), vec3(0,0,1), 
                                vec3(0,0,1),  vec3(0,0,1)];   // ...
        this.texture_coords = [ vec2(0.5,0.5),   vec2(1,0.5),   vec2(0.5,1), 
                                 vec2(0,0.5),   
                                vec2(0.5,0),
                              ];   // ...
        this.indices        = [0, 1, 2,  0,2,3,   0,3,4,  
...
Declare_Any_Class( "Tail",   // An alternative without two separate sections
  { populate( rows, columns )
      { Surface_Of_Revolution.prototype.insert_transformed_copy_into( this, [ rows, columns, [ vec3( 1, 1, 1 ), vec3( 1, 0, -1 ), vec3( 0, 0, -1 )  ] ] ) ; } }, Shape )
