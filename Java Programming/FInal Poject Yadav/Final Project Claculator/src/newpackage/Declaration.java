...
private Declaration(Declaration parent) {
...
public Declaration push(Object ... args) {
	for (Object obj : args)
		this.list.add(obj);
	return this;
}
...
public BigDecimal eval()
	throws SyntaxError, InvalidInpu, UnknownOperator {
	Object obj = null;
	Declaration curr = this;
	...
	if (!this.equals(curr))
		throw new SyntaxError("Unmatched brackets");
	...
	for (int i = 0; i < list.size(); i++) {
		...
		if (isOperator(obj)) switch ((byte) obj) {
			...
			case CUBE_ROOT:
				...
				else if (isExpression(obj)) rhs = ((Declaration) obj).eval();
				else throw new SyntaxError("Missing operand");
			...
			case POWER:
				...
				else if (isExpression(obj)) lhs = ((Declaration) obj).eval();
				else throw new SyntaxError("Missing operand");
				obj = i + 1 < list.size() ? list.get(i + 1) : -1;
			...
			case FACTORIAL:
				...
				//	throw new InvalidInpu("Factorial input less than zero");
				//else if (lhs.compareTo(new BigDecimal(5000)) > 0)
				//	throw new InvalidInpu("Factorial input too large (>5000)");
	...
	for (int i = 0; i < list.size(); i++) {
			...
			s == 1 && (obj.equals(ADD) || obj.equals(SUB)))
		{
			...
			list.set(i, rhs = ((Declaration) obj).eval());
			obj = i > 0 ? list.get(i - 1) : -1;
			if (isOperand(obj)) {
	...
	while (list.size() > 1) {
		obj = list.get(0);
		if (isExpression(obj))
			lhs = ((Declaration) obj).eval();
		else if (isOperand(obj))
			lhs = (BigDecimal) obj;
		else throw new UnknownOperator();
		obj = list.get(1);
		if (isExpression(obj))
			rhs = ((Declaration) obj).eval();
		else if (isOperand(obj))
			rhs = (BigDecimal) obj;
		else throw new UnknownOperator();
	...
	if (list.size() == 0)
		...
		list.set(0, ((Declaration) list.get(0)).eval());
